...
import java.io.*;
class StringClassAndStringBuffer
 {
	public static void main(String[] args)
	{
	StringBuffer s = new StringBuffer("Ameena");
		int p = s.length();
		int q = s.capacity();
		System.out.println("Length of string Ameena=" + p);
		System.out.println("Capacity of string Ameena=" + q);
    s.append(" Shajahan");
    System.out.println("After Appending="+s);
    s.insert(11, "-");
    System.out.println("After inserting - @position 9="+s);
    s.reverse();
    System.out.println("After reversing ="+s);
    s.delete(0, 2);
            System.out.println("After deteting the array from 0 to 2 position="+s);
            s.deleteCharAt(1);
            System.out.println("After deleting a character @1 position="+s);
            s.replace(0,5,"a");
          ...
          char c=s1.charAt(5);/*returns the char value at the 2th index of string "welcome to java programming" */
           ...
           s1=s1.concat(" ... let's crack it together");
